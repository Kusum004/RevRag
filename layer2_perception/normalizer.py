...
def parse_ui_hierarchy(xml_content: str, screen_width: int = 1080, screen_height: int = 2400) -> List[UIElement]:
    ...
    def traverse(node: ET.Element, depth: int):
        ...
        editable = attrib.get("focused", "false").lower() == "true" or "edittext" in class_name.lower()
        ...
        if bounds and bounds.width > 0 and bounds.height > 0:
            ...
            if bounds.left < screen_width and bounds.top < screen_height and bounds.right > 0 and bounds.bottom > 0:
                ...
                # Check if this element represents a tab, button, or primary action
                ...
                # Interactivity: explicitly clickable node, button class, input, or explicit action/tab keyword
                is_clickable = node_clickable or ("button" in class_name.lower()) or has_action_keyword

                # Exclude purely static read-only text labels from being clickable unless explicitly clickable
                if "textview" in class_name.lower() and not node_clickable and not has_action_keyword:
                    is_clickable = False
